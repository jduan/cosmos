...
// program, like erroneous input, a network outage, or a permissions problem. That such situations
// occur is not up to us; even a bug-free program will encounter them from time to time. Most of
// this chapter is dedicated to that kind of error.
//
// Panic is for the other kind of error, the kind that "should never happen"! A program panics when
// it encounters something so messed up that there must be a bug in the program itself, such as:
...
// panic. Even if you do use threads or catch_unwind(), all your panic-handling code will likely be
// concentrated in a few places. It’s unreasonable to expect every function in a program to
